      ...
      if(querySnapshot.size>1) {
        ...
        for(var doc in querySnapshot.docs){
          if(doc.data()['uid']!=widget.currUserMap['uid']){
            setState(()=>{
              results.add(ChatTile(receiverUserMap: doc.data(), senderUserMap: widget.currUserMap, chatId: chatId(doc.data()['uid'],widget.currUserMap['uid']), controller: _controller, isSearch: true, check: true,add: () async{
                ...
                firestore.collection('group').doc(widget.currUserMap['uid']+widget.groupName).collection('members').doc(doc.data()['uid']).set(doc.data());
